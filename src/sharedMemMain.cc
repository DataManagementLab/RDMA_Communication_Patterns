...
int benchmarkDriver(){
   ...
   // create shared storage client and connec to receiver
   
   BenchmarkSender<SenderP, SenderT, PL_SIZE> sharedMemorySender(FLAGS_totalReceiver, FLAGS_requests);
   auto recvIps = sharedMemorySender.queryReceiverInformation(); 

   for(auto& receiverIP : recvIps ){
      size_t nodeId {0};
      sharedMemorySender.connect(receiverIP,nodeId);
   }

   // allocate shared memory for responses

   Response<PL_SIZE>* sharedResponseArea = static_cast<Response<PL_SIZE>*>(sharedMemorySender.localAlloc(sizeof(Response<PL_SIZE>) * FLAGS_localSender));

   
   WorkerGroup group(FLAGS_localSender);
   Barrier barrier(group.size());
   
   group.run([&barrier, &senderLatencies , &sharedResponseArea, &sharedMemorySender](int workerId){
                ...
                sender.benchmarkPhase(senderLatencies, workerId *FLAGS_requests, &sharedResponseArea[workerId], sharedMemorySender.convertPointerToOffset((void*) &sharedResponseArea[workerId]), sharedMemorySender.getOwnNodeID());
...
int main(int argc, char *argv[])
{
   ...
   // Not supported in this mode since in order to share memory for Incomcing sends we would need a dispatcher, to poll on the RQ which is the other benchmark
